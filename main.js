let apiKey = "f82d178d2f69780470c7099c57538baa";
...
// Show the modal when the Notify button is clicked
...
// Save the temperature and store it
saveTempButton.addEventListener('click', () => {
  ...
  // Save the notification temperature (in local storage or a variable)
  localStorage.setItem('notificationTemp', tempValue);
  
  // Close the modal
  notifyModal.style.display = 'none';
  alert(`You will now be notified when the temperature reaches ${tempValue}°C.`);
});
...
const showNotification = (title, message) => {
  if ('Notification' in window && Notification.permission === 'granted') {
    new Notification(title, {
      body: message,
      icon: "./img/sun.png", // Update the icon path as needed
    });
  }
};
...
function displayForecast(data) {
  ...
  for (const date in dailyForecasts) {
    const icons = {
      ...
      clear: "./img/sun.png",
      snow: "./img/snow.png",
      clouds: "./img/cloud-30.webp",
      mist: "./img/mist.png",
      haze: "./img/haze.png"
    };
    const imgSrc = icons[dailyForecasts[date].weatherImg] || "./img/sun.png";
  ...
  // Apply scroll animation to forecast boxes
...
navigator.geolocation.getCurrentPosition(async function (position) {
    ...
    try {
        ...
        if (weatherCondition === "rain") {
            ...
            weatherImg.src = "./ img/cloud-30.webp";
            weatherImgs.src = "./ img/cloud-30.webp";
